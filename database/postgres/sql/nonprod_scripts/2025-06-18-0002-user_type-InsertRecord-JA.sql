...
CREATE OR REPLACE PROCEDURE CreateOrUpdateUserTypeTable()
LANGUAGE plpgsql
AS $$
BEGIN
   -- Check if 'SUPER_ADMIN' exists
   IF NOT EXISTS (
        SELECT 1 FROM "user_type" WHERE "name" = 'SUPER_ADMIN'
    )
    THEN        INSERT INTO "user_type" (id, "name", description, "crUser", "crDate", "modUser", "modDate")
   ...
   IF NOT EXISTS (
        SELECT 1 FROM "user_type" WHERE "name" = 'CLIENT_ADMIN'
    )
    THEN        INSERT INTO "user_type" (id, "name", description, "crUser", "crDate", "modUser", "modDate")
    ...
    END IF;   -- Check if 'CLINICAL_STAFF' exists
   IF NOT EXISTS (
        SELECT 1 FROM "user_type" WHERE "name" = 'CLINICAL_STAFF'
    )
    THEN        INSERT INTO "user_type" (id, "name", description, "crUser", "crDate", "modUser", "modDate")
    ...
    END IF;   -- Check if 'OFFICE_STAFF' exists
   IF NOT EXISTS (
        SELECT 1 FROM "user_type" WHERE "name" = 'OFFICE_STAFF'
    )
    THEN        INSERT INTO "user_type" (id, "name", description, "crUser", "crDate", "modUser", "modDate")
    ...
    END IF;   -- Check if 'PATIENT' exists
   IF NOT EXISTS (
        SELECT 1 FROM "user_type" WHERE "name" = 'PATIENT'
    )
    THEN        INSERT INTO "user_type" (id, "name", description, "crUser", "crDate", "modUser", "modDate")
...
CALL CreateOrUpdateUserTypeTable();
